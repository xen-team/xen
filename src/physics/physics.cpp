...
void Physics::start()
{
    auto& registry = scene->get_entity_manager().get_registry();
    auto view = registry.view<Rigidbody, KinematicCharacter>();
    auto fpsview = registry.view<FpsPlayer>();

    fpsview.each([](FpsPlayer& playa) { playa.start(); });

    view.each([](Rigidbody& rigidbody, KinematicCharacter& kc) {
        rigidbody.start();
        kc.start();
    });
}
...
void Physics::update()
{
    ...
    // Rewrite this to more dynamic and modular system, smth with registering components according to systems
    fpsview.each([](FpsPlayer& playa) { playa.update(); });
    ...
    dynamics_world->stepSimulation(Engine::get()->get_delta().as_seconds());
    check_for_collision_events();
}

Raycast Physics::raytest(Vector3f const& start, Vector3f const& end) const
{
...
void Physics::set_gravity(Vector3f const& gravity)
{
    this->gravity = gravity;
    dynamics_world->setGravity(Collider::convert(gravity));
}

void Physics::set_air_dencity(float air_density)
{
...
void Physics::check_for_collision_events()
{
