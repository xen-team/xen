...
public:
    ...
    constexpr explicit Matrix2(float x1, float x2, float y1, float y2);

    constexpr explicit Matrix2(std::array<float, static_cast<std::size_t>(COUNT* COUNT)> src);

    constexpr explicit Matrix2(std::array<row_t, COUNT> src);
