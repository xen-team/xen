...
void callback_drop(GLFWwindow* glfw_window, int32_t count, const char** paths) {
	auto window = static_cast<Window*>(glfwGetWindowUserPointer(glfw_window));
...
void callback_key(GLFWwindow* glfw_window, int32_t key, [[maybe_unused]] int32_t scancode, int32_t action, int32_t mods) {
	auto window = static_cast<Window*>(glfwGetWindowUserPointer(glfw_window));
	window->on_key(static_cast<Key>(key), static_cast<InputAction>(action), InputMod(mods));
}
...
void callback_mouse_button(GLFWwindow* glfw_window, int32_t button, int32_t action, int32_t mods) {
	auto window = static_cast<Window*>(glfwGetWindowUserPointer(glfw_window));
	window->on_mouse_button(static_cast<MouseButton>(button), static_cast<InputAction>(action), InputMod(mods));
}
...
float Window::smooth_scroll_wheel(double value, float delta) {
	if (value != 0.0) {
		value -= static_cast<float>(delta) * std::copysign(3.0, value);
...
Window::Window(window_id id) : id{id}, size{default_size}, title{default_name}, resizable{true}, focused{true} {
...
void Window::set_size(const Vector2ui& size) {
    this->size = size;
    glfwSetWindowSize(window, size.x, size.y);
     
}

void Window::set_pos([[maybe_unused]] const Vector2ui& pos) {
	#ifndef PLATFORM_WAYLAND
    this->pos = pos;
    glfwSetWindowPos(window, pos.x, pos.y);
	#else
...
#endif
		glfwSetWindowMonitor(window, selected->get_monitor(), 0, 0, video_mode.width, video_mode.height, GLFW_DONT_CARE);
	} else {
#ifdef ACID_DEBUG
...
int32_t OverlappingArea(Vector2i const& l1, Vector2i const& r1, Vector2i const& l2, Vector2i const& r2) {
...
Monitor const* Window::get_current_monitor() const {
	...
	for (auto const& monitor : Windows::get()->get_monitors()) {
		where = ranked_monitor.insert(where, {OverlappingArea(monitor->get_workarea_pos(), monitor->get_workarea_pos() + monitor->get_workarea_size(),
			pos, pos + size), monitor.get()});
	}
