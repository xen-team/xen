...
void callback_monitor(GLFWmonitor* glfw_monitor, int32_t event) {
	...
	if (event == GLFW_CONNECTED) {
		auto monitor = monitors.emplace_back(std::make_unique<Monitor>(glfw_monitor)).get();
		Windows::get()->on_monitor_connect(monitor, true);
	} else if (event == GLFW_DISCONNECTED) {
		for (const auto &monitor : monitors) {
			if (monitor->get_monitor() == glfw_monitor) {
				Windows::get()->on_monitor_connect(monitor.get(), false);
			}
		}

		monitors.erase(std::remove_if(monitors.begin(), monitors.end(), [glfw_monitor](const auto &monitor) {
			return glfw_monitor == monitor->get_monitor();
		}));
	}
}
...
Window const* Windows::get_window(window_id id) const {
	if (id >= windows.size())
		return nullptr;
	return windows[id].get();
}

Window* Windows::get_window(window_id id) {
	if (id >= windows.size())
		return nullptr;
	return windows[id].get();
}
