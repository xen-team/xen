...
public:
	...
	rocket::signal<void(Key, InputAction, InputMod)> on_key;
	rocket::signal<void(char)> on_char;
	rocket::signal<void(MouseButton, InputAction, InputMod)> on_mouse_button;
...
public:
    ...
    Vector2ui const& get_size() const { return size; }

    uint16_t get_width() const { return size.x; }

    uint16_t get_height() const { return size.y; }

    void set_size(Vector2ui const& size);

    float get_aspect_ratio() const { return static_cast<float>(get_width()) / static_cast<float>(get_height()); }

    Vector2ui const& get_pos() const { return pos; }
	...
	bool is_iconified() const { return iconified; }
	...
	bool is_window_selected() const { return window_selected; }

	bool is_cursor_hidden() const { return cursor_hidden; }

	void set_cursor_hidden(bool hidden);

	// void set_cursor(const Cursor *cursor);

	InputAction get_key(Key key) const;

	InputAction get_mouse_button(MouseButton mouseButton) const;

	Vector2d const& get_mouse_pos() const { return mouse_pos; }

	void set_mouse_pos(Vector2d const& mousePosition);

	Vector2d const& get_mouse_pos_delta() const { return mouse_pos_delta; }

	Vector2d const& get_mouse_scroll() const { return mouse_scroll; }

	void set_mouse_scroll(Vector2d const& scroll);

	Vector2d const& get_mouse_scroll_delta() const { return mouse_scroll_delta; }

	static std::string_view key_as_str(Key key);

	Monitor const* get_current_monitor() const;
...
private:
	...
	friend void callback_drop(GLFWwindow *glfw_window, int32_t count, const char **paths);
	...
	static float smooth_scroll_wheel(double value, float delta);
};
}