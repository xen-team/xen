...
public:
    ...
    template <typename K = float>
    [[nodiscard]] auto rotate(K const& angle) const;

    template <typename K = float, typename J>
    [[nodiscard]] auto rotate(K const& angle, Vector2<J> const& rotationAxis) const;
    ...
    template <typename K>
    [[nodiscard]] auto distance(Vector2<K> const& other) const;
    ...
    [[nodiscard]] auto cartesian_to_polar() const;

    [[nodiscard]] auto polar_to_cartesian() const;
