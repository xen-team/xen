...
public:
	...
	constexpr static Time seconds(T seconds) {
        return Time(std::chrono::duration<T>(seconds));
    }

	template<typename T = internal_type>
    constexpr static Time milliseconds(T milliseconds) {
        return Time(std::chrono::duration<T>(milliseconds));
    }

	template<typename T = internal_type>
    constexpr static Time microseconds(T microseconds) {
        return Time(std::chrono::duration<T>(microseconds));
    }

	template<typename T = float>
	constexpr T as_seconds() const {
		return static_cast<T>(value.count() / 1'000'000);
	}

	template<typename T = float>
	constexpr T as_milliseconds() const {
		return static_cast<T>(value.count() / 1'000);
	}

	template<typename T = internal_type>
	constexpr T as_microseconds() const {
		return static_cast<T>(value.count());
	}

	static Time now() {
		static auto const local_epoch = std::chrono::high_resolution_clock::now();
		return std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - local_epoch);
	}

	static std::string get_date_time(std::string_view format = "%Y-%m-%d %H:%M:%S") {
