...
int const MAX_BONES = 100;
int const MAX_BONES_PER_VERTEX = 4;
uniform mat4 bonesMatrices[MAX_BONES];

void main()
{
    mat4 boneTransform = bonesMatrices[boneIds[0]] * weights[0] + bonesMatrices[boneIds[1]] * weights[1] +
                         bonesMatrices[boneIds[2]] * weights[2] + bonesMatrices[boneIds[3]] * weights[3];
    ...
    if (hasDisplacement) {
        mat3 inverseTBN =
            transpose(TBN); // Calculate matrix to go from world -> tangent (orthogonal matrix's transpose = inverse)
...
vec3 CalculateDirectionalLightRadiance(
    vec3 albedo, vec3 normal, float metallic, float roughness, vec3 fragToViewNorm, vec3 baseReflectivity
);
...
void main()
{
    ...
    vec3 emission = hasEmission ?
                        (material.hasEmissionTexture ? texture(material.texture_emission, textureCoordinates).rgb :
                                                       material.emissionColour) :
                        vec3(0.0);
    ...
    // Sample textures
    vec4 sampledAlbedo = material.hasAlbedoTexture ?
                             texture(material.texture_albedo, textureCoordinates).rgba * material.albedoColour :
                             material.albedoColour;
    ...
    float metallic =
        material.hasMetallicTexture ? texture(material.texture_metallic, textureCoordinates).r : material.metallicValue;
    float unclampedRoughness = material.hasRoughnessTexture ?
                                   texture(material.texture_roughness, textureCoordinates).r :
                                   material.roughnessValue; // Used for indirect specular (reflections)
    float roughness =
        max(unclampedRoughness,
            0.04); // Used for calculations since specular highlights will be too fine, and will cause flicker
    float ao = texture(material.texture_ao, textureCoordinates).r;

    // Normal mapping code. Opted out of tangent space normal mapping since I would have to convert all of my lights to
    // tangent space
    ...
    directLightIrradiance +=
        CalculateDirectionalLightRadiance(albedo, normal, metallic, roughness, fragToViewNorm, baseReflectivity);
    directLightIrradiance +=
        CalculatePointLightRadiance(albedo, normal, metallic, roughness, fragToViewNorm, baseReflectivity);
    directLightIrradiance +=
        CalculateSpotLightRadiance(albedo, normal, metallic, roughness, fragToViewNorm, baseReflectivity);
...
)
{
    ...
    for (int i = 0; i < numDirPointSpotLights.x; ++i) {
        ...
        float denominator = 4 * max(dot(fragToViewNorm, normal), 0.1) * max(dot(lightDirNorm, normal), 0.0) +
        ...
        directLightIrradiance +=
            (diffuse + specular) * radiance * max(dot(normal, lightDirNorm), 0.0) * (1.0 - shadowAmount);
    }
...
)
{
    ...
    for (int i = 0; i < numDirPointSpotLights.y; ++i) {
        ...
        // Finally calculate the specular part of the Cook-Torrance BRDF (max 0.1 stops any visual artifacts)
        vec3 numerator = specularRatio * normalDistribution * geometry;
        float denominator = 4 * max(dot(fragToViewNorm, normal), 0.1) * max(dot(fragToLightNorm, normal), 0.0) +
                            0.001; // Prevents any division by zero
        vec3 specular = numerator / denominator;
        ...
        pointLightIrradiance +=
            (diffuse + specular) * radiance * max(dot(normal, fragToLightNorm), 0.0) * (1.0 - shadowAmount);
    }
...
float GeometrySmith(vec3 normal, vec3 viewDirNorm, vec3 lightDirNorm, float roughness)
{
    return GeometrySchlickGGX(max(dot(normal, viewDirNorm), 0.0), roughness) *
           GeometrySchlickGGX(max(dot(normal, lightDirNorm), 0.0), roughness);
}
float GeometrySchlickGGX(float cosTheta, float roughness)
{
...
float CalculateDirLightShadow()
{
    ...
    // Perform Percentage Closer Filtering (PCF) in order to produce soft shadows - Use bilinear filtering to get some
    // free samples (4 bilinear samples, 4 samples -> 16 values actually processed)
    float shadow = 0.0;
    ...
    for (float y = -1.5; y < 1.0; y += 2.0) {
        for (float x = -1.5; x < 1.0; x += 2.0) {
            ...
            shadow += currentDepth > sampledDepthPCF + dirLightShadowData.shadowBias ?
                          1.0 :
                          0.0; // Add shadow bias to avoid shadow acne. However too much bias can cause peter panning
        }
    }
    shadow *= 0.25;
...
float CalculateSpotLightShadow()
{
    ...
    for (float y = -1.5; y < 1.0; y += 2.0) {
        for (float x = -1.5; x < 1.0; x += 2.0) {
            ...
            shadow += currentDepth > sampledDepthPCF + spotLightShadowData.shadowBias ?
                          1.0 :
                          0.0; // Add shadow bias to avoid shadow acne. However too much bias can cause peter panning
        }
    }
...
vec2 ParallaxMapping(vec2 texCoords, vec3 viewDirTangentSpace)
{
    // Figure out the LoD we should sample from while raymarching the heightfield in tangent space. Required to fix an
    // artifacting issue
    ...
    float const minSteps = minMaxDisplacementSteps.x;
    float const maxSteps = minMaxDisplacementSteps.y;
    float numSteps =
        mix(maxSteps, minSteps, clamp(expectedLod * 0.4, 0, 1)
        ); // More steps are required at lower mip levels since the camera is closer to the surface
    ...
    // Keep ray marching along vector p by the texture coordinate delta, until the raymarching depth catches up to the
    // sampled depth (ie the -view vector intersects the surface)
    ...
    float beforeDepth =
        textureLod(material.texture_displacement, prevTexCoords, lodToSample).r - currentLayerDepth + layerDepth;
