/*
        Deferred SSAO Generation. It tiles a 4x4 texture accross the screen, in order to elimate banding but then it
   will introduce high frequency noise that can be blurred
*/
...
void main()
{
    ...
    // Make a TBN matrix to go from tangent -> world space (so we can put our hemipshere tangent sample points into
    // world space) Since our normal is already in world space, this TBN matrix will take our tangent space vector and
    ...
    for (int i = 0; i < numKernelSamples; ++i) {
        ...
        // Check if our current samples depth is behind the screen space geometry's depth, if so then we know it is
        // occluded in screenspace
        float sceneDepth = texture(depthTexture, sampleScreenSpace.xy).r;

        // Peform a range check on the current fragment we are calculating the occlusion factor for, and the occlusion
        // position
