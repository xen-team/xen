...
void main()
{
    ...
    // Cut off the translation part of the view matrix (so the cube will not be translated because we want the cube to
    // surround the probe's origin)
...
void main()
{
    ...
    for (float phi = 0.0; phi < PI * 2.0; phi += delta) {           // Ring of the hemisphere
        for (float theta = 0.0; theta < PI * 0.5; theta += delta) { // For the increasing rings on the hemisphere
            ...
            // Scaled by sin(theta) to account for smaller sample areas in the higher hemipshere rings, and scaled by
            // cos(theta) due to light being weaker at larger angles
