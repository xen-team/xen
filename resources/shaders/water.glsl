...
void main()
{
    ...
    if (clearWater) {
        ...
        distortion =
            (texture(dudvWaveTexture, vec2(planeTexCoords.x + waveMoveFactor, planeTexCoords.y)).rg * 2.0 - 1.0) *
            0.1; // Unpack the dudv map
    ...
    if (clearWater) {
        ...
        normal = normalize(vec3(normal.r * 2.0 - 1.0, normal.b * waterNormalSmoothing, normal.g * 2.0 - 1.0)
        ); // Normal maps are in tangent space but we know tangent space z component = y component in world space since
           // plane is always flat and assume normal is (0, 1, 0)
    }
    ...
    if (shouldShine) {
        ...
        for (int i = 0; i < numDirPointSpotLights.y; i++) {
            ...
            float attenuation =
                (falloffNumerator * falloffNumerator) / ((lightToFragDistance * lightToFragDistance) + 1.0);

            specHighlight +=
                pointLights[i].intensity * pointLights[i].lightColour * specular * attenuation * dampeningEffect2;
        }
        ...
        for (int i = 0; i < numDirPointSpotLights.z; i++) {
            ...
            float attenuation =
                intensity * (falloffNumerator * falloffNumerator) / ((lightToFragDistance * lightToFragDistance) + 1.0);

            specHighlight +=
                spotLights[i].intensity * spotLights[i].lightColour * specular * attenuation * dampeningEffect2;
        }
    }
