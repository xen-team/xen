...
vec3 CalculateDirectionalLightRadiance(
    vec3 albedo, vec3 normal, float metallic, float roughness, vec3 fragPos, vec3 fragToViewNorm, vec3 baseReflectivity
);
...
void main()
{
    ...
    if (emissiveIntensity > 0.0) {
        ...
        color = sampledAlbedo * emissiveIntensity *
                255.0; // emissiveIntensity is a float [0.0, 1.0] but we specify intensity as a u8 [0, 255]
        return;
    }
    ...
    directLightIrradiance += CalculateDirectionalLightRadiance(
        albedo, normal, metallic, roughness, fragPos, fragToViewNorm, baseReflectivity
    );
    directLightIrradiance +=
        CalculatePointLightRadiance(albedo, normal, metallic, roughness, fragPos, fragToViewNorm, baseReflectivity);
    directLightIrradiance +=
        CalculateSpotLightRadiance(albedo, normal, metallic, roughness, fragPos, fragToViewNorm, baseReflectivity);
...
float GeometrySmith(vec3 normal, vec3 fragToViewNorm, vec3 lightDirNorm, float roughness)
{
    return GeometrySchlickGGX(max(dot(normal, fragToViewNorm), 0.0), roughness) *
