...
void main()
{
    ...
    vec4 albedo = material.hasAlbedoTexture ?
                      texture(material.texture_albedo, textureCoordinates).rgba * material.albedoColour :
                      material.albedoColour;

    // If we have emission, hijack the albedo and replace it with the emission colour. Then since we want HDR values and
    // albedo RT is LDR, we can store the emission intensity in the alpha of the gb_MaterialInfo RT
    ...
    if (hasEmission) {
        if (material.hasEmissionTexture) {
            ...
            // Check emission map sample, if it is black (ie. no emission) then we just skip emission for this fragment,
            // otherwise hijack the albedo RT
    ...
    float roughness = material.hasRoughnessTexture ? texture(material.texture_roughness, textureCoordinates).r :
                                                     material.roughnessValue;
    float ao = texture(material.texture_ao, textureCoordinates).r;
    float emissionIntensity = overwriteAlbedoWithEmission ? material.emissionIntensity / 255.0 :
                                                            0.0; // Converting u8 [0, 255] -> float [0.0, 1.0]
