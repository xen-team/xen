...
bool ImGui::SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1, float* br_size, float min_size1, float min_size2, float hover_extend, float hover_visibility_delay, ImU32 bg_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    if (held)
    {
        ...
        float size_2_maximum_delta = ImMax(0.0f, *br_size - min_size2);
        ...
        if (mouse_delta != 0.0f)
        {
            ...
            *br_size = ImMax(*br_size - mouse_delta, min_size2);
